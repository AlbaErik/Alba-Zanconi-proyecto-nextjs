  ...
  return (
    <div className="card rounded-lg bg-white shadow-md">
      ...
      <Link href={{pathname: `/user/product/${id}`,}} 
              className="flex px-[5%] text-xl font-semibold tracking-tight text-gray-900 hover:underline">{title}</Link>
      <div className="flex items-center justify-between mb-3 mt-3">
        <h3 className="pl-[5%] text-3xl font-bold text-gray-900 ">{"$"+price}</h3>
        <button onClick={() => setState([])} className="mr-[5%] pr-3 pl-3 overflow-hidden inline-block align-middle text-center text-white hover:bg-cyan-800 bg-cyan-700 rounded-lg h-8">
          Agregar
        </button>
      </div>
